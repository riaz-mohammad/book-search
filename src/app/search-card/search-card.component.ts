  ...
  template: `
    ...
    <ng-container *ngFor="let book of books; let i = index">
      ...
      <ng-container *ngIf="i < 8 && (book | bookThumbnail) as source">
         <app-card (click)="expansion(book)" expand #card>
          <app-card-horizontal *ngIf="!card.cardExpanded && horizontal" 
                              [book]="book" 
                              [source]="source">
          </app-card-horizontal>

          <app-card-vertical *ngIf="card.cardExpanded && vertical" [book]="book">
          </app-card-vertical>
        </app-card>
